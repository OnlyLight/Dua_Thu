...
public class Main2Activity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        data.queryData("ALTER TABLE Lich MODIFY ThoiGian VARCHAR(150) NOT NULL UNIQUE");
    ...
    private void xacNhanUpdate(String thoiGian, boolean sang, boolean chieu, boolean toi, int id) {
        String buoiSang = "null", buoiChieu = "null", buoiToi = "null";
        if (sang) buoiSang = "Sáng";
        if (chieu) buoiChieu = "Chiều";
        if (toi) buoiToi = "Tối";

        data.queryData("UPDATE Lich SET ThoiGian = '" + thoiGian + "', BuoiSang = '" + buoiSang + "', BuoiChieu = '" + buoiChieu + "', BuoiToi = '" + buoiToi + "' WHERE Id = '" + id + "'");
...
//        }
    ...
    private void check() {
        String sang = "null", chieu = "null", toi = "null";
        if (cbSang.isChecked()) sang = "Sáng";
        if (cbChieu.isChecked()) chieu = "Chiều";
        if (cbToi.isChecked()) toi = "Tối";

        if (sang.equals("null") && chieu.equals("null") && toi.equals("null")) {
            Toast.makeText(this, "Chưa chọn buổi !! Xin chọn lại", Toast.LENGTH_SHORT).show();
        }

        if (edtDate.getText().toString().length() > 0) {
            data.queryData("INSERT INTO Lich VALUES(null, '" + format.format(calendar.getTime()) + "', '" + sang + "', '" + chieu + "', '" + toi + "')");
